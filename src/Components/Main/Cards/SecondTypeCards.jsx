...
function SecondTypeCards({ filter }) {
        ...
        {filteredCards.map((card, idx) => (
          <Card key={card.id} sx={{ minHeight: "410px", width: "42rem" }}>
            ...
            <div
              ...
              onMouseEnter={() => setHoveredRating2(card.id || idx)}
              onMouseLeave={() => setHoveredRating2(null)}
            >
              <IconButton className="secondRateIconButton">
                {hoveredRate2 === (card.id || idx) ? (
                  <HoverRating
                    value={rates2[card.id || idx]}
                    onChange={(value) => {
                      setRates2((prevRates) => ({
                        ...prevRates,
                        [card.id || idx]: value,
...
export default SecondTypeCards;
